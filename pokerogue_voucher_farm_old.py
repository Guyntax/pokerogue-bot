...
def main():
    ...
    while match_and_locate_template(ss, load_screen_im) < CONFIDENCE_THRESHOLD:
        print("Load first battle")
        if counter >= 10 and match_and_locate_template(ss, continue_im) >= CONFIDENCE_THRESHOLD:
            go_to_load_game_menu()
        time.sleep(0.5)
        ss = get_screenshot()
        counter += 1
    ...
    while match_and_locate_template(ss, what_will_im, low_mask) < CONFIDENCE_THRESHOLD:
        print("First battle - Start")
        if counter >= 20:
            second_battle()
        if counter >= 10 and match_and_locate_template(ss, load_screen_im) >= CONFIDENCE_THRESHOLD:
            load_penultimate_game()
        keyboard.press_and_release('x')
        time.sleep(0.5)
        ss = get_screenshot()
    ...
    while match_and_locate_template(ss, skip_an_item_im, low_mask) < CONFIDENCE_THRESHOLD:
        ...
        if counter >= 20:
            second_battle()
        if counter >= 10 and match_and_locate_template(ss, what_will_im, low_mask) >= CONFIDENCE_THRESHOLD:
            attack()
        time.sleep(MIN_DELAY)
        keyboard.press_and_release('x')
        time.sleep(0.5)
    ...
    counter = 0
    while match_and_locate_template(ss, what_will_im, low_mask) < CONFIDENCE_THRESHOLD:
        print("First battle - Completed")
        if counter >= 20:
            second_battle()
        if counter >= 10 and match_and_locate_template(ss, skip_an_item_im, low_mask) >= CONFIDENCE_THRESHOLD:
            select_reward_voucher()
        time.sleep(0.5)
        ss = get_screenshot()
        counter += 1
    ...
    def second_battle():
        ...
        while match_and_locate_template(ss, side_menu_im) < CONFIDENCE_THRESHOLD:
            print("Second battle - Save and quit")
            ss = get_screenshot()
            time.sleep(0.5)
            counter += 1
